...
CachedRequest::CachedRequest(const std::shared_ptr<TTLCache>& cache,
                             const std::shared_ptr<Request>& slow_request,
                             const std::shared_ptr<RequestTypes>& req_types,
                             int max_fetch_retries)
: d_cache{cache}
, d_slow_request{slow_request}
...
std::string CachedRequest::query_impl(const Data& request_data)
{
    ...
    std::string key = create_query_str(request_data);
    ...
    for (int i = 0; i < d_max_attempts; ++i) {
        try {
            ...
            std::future<std::string> value_promise = d_slow_request->query(request_data);
...
std::string CachedRequest::create_query_str(const Data& data) const
{
    return d_slow_request->create_query_str(data);
}
